...
Run via:  poe gen-lgus   (or `uv run python docs/hooks/generate_lgu_pages.py`)
...
def load_db():
    ...
    # Force the lazy Database to load the bundled PSGC dataset.
...
def render_index(lgus: list[dict]) -> str:
    ...
    for lgu in lgus:
        ...
        items.append(f"- [{label}](./{slug}.md) — {lgu['barangay_count']} barangays")
    listing = "\n".join(items)
    return f"""---
...
> ({DATA_VERSION}). To regenerate or extend the list, run `poe gen-lgus`.

See the [full package documentation](../index.md) for fuzzy search, address
...
def main() -> None:
    ...
    # Clear previous generated pages
    for old in LGU_DIR.glob("*.md"):
        old.unlink()

    for lgu in lgus:
        (LGU_DIR / f"{lgu['slug']}.md").write_text(render_page(lgu), encoding="utf-8")

    (LGU_DIR / "index.md").write_text(render_index(lgus), encoding="utf-8")
