...
`search()` returns raw dicts and will be removed in 2027.X.X.X. Use `validate()` or `search_fuzzy()` for typed results. See [Migrate from the legacy API](../how-to/migrate-from-legacy-api.qmd).
:::
...
::: {.callout-warning title="Deprecated"}
`search()` returns raw dicts and will be removed in 2027.X.X.X. Use `search_fuzzy()` for typed results. See [Migrate from the legacy API](../how-to/migrate-from-legacy-api.qmd).
:::
...
::: {.callout-warning title="Deprecated"}
`search()` returns raw dicts and will be removed in 2027.X.X.X. Use `search_fuzzy()` or `validate_many()` for typed results. See [Migrate from the legacy API](../how-to/migrate-from-legacy-api.qmd).
:::
...
- [Search reference](../reference/python-api/search.qmd) — full `search()` parameter documentation
