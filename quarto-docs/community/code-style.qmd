...
- **Deprecated APIs stay importable but are not used in new code.** The dict-based `search()`, the `BARANGAY` / `BARANGAY_EXTENDED` / `BARANGAY_FLAT` aliases, and `BarangayModel` are present for compatibility — new work uses [`search_fuzzy`](../reference/python-api/search.qmd) and the [`Database`](../reference/python-api/records.qmd) views.
