"""Append generated LGU landing pages to quarto-docs/llms-full.txt.

After running `poe gen-quarto-lgus`, run `poe gen-quarto-llms` so LLMs that
consume llms-full.txt can surface the per-LGU pages (e.g. "list of barangays in
...
def load_index_titles() -> list[tuple[str, str]]:
    index_path = LGU_DIR / "index.qmd"
    if not index_path.exists():
        return []
    text = index_path.read_text(encoding="utf-8")
    titles = re.findall(r"^\- \[(.*?)\]\(\./(.*?)\.qmd\)", text, flags=re.MULTILINE)
    return titles
...
def main() -> None:
    ...
    for label, slug in titles:
        lines.append(f"- [{label}]({SITE_URL}/locations/{slug}/)")
    lines.append("")
